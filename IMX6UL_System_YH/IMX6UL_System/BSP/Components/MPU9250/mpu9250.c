...
static BOOL MPU_Write_Len(u8 addr,u8 reg,u8 len,u8 *buf)
{
    int i;
    ...
    pIIC->Start();
    //发送器件地址+写命令
    if(!pIIC->WriteByte((addr<<1)|0,IIC_ACK))
    {
    	pIIC->Stop();
    ...
    for(i=0;i<len;i++)
    {
    	if(!pIIC->WriteByte(buf[i],IIC_ACK))
    	{
    		pIIC->Stop();
...
static BOOL MPU_Read_Len(u8 addr,u8 reg,u8 len,u8 *buf)
{
	pIIC->Open();
	...
	pIIC->Start();
	//发送器件地址+读命令
    pIIC->WriteByte((addr<<1)|1,IIC_ACK);
    while(len > 0)
    {
        if(len == 1)
        {
        	//读数据,发送nACK
        	*buf++=pIIC->ReadByte(IIC_NoACK);
        }
		else
		{
			//读数据,发送ACK
			*buf++=pIIC->ReadByte(IIC_ACK);
		}
		len--;
    }
...
static BOOL MPU_Write_Byte(u8 addr,u8 reg,u8 data)
{
	pIIC->Open();
...
static u8 MPU_Read_Byte(u8 addr,u8 reg)
{
	u8 dat;
...
//返回值:0,成功
//    其他,错误代码
BOOL MPU9250_Init(const struct i2c_ops *iic)
{
    u8 res=0;
...
//返回值:0,成功
//    其他,错误代码
BOOL MPU_Get_Gyroscope(s16 *gx,s16 *gy,s16 *gz)
{
    u8 buf[6],res; 
...
//返回值:0,成功
//    其他,错误代码
BOOL MPU_Get_Accelerometer(s16 *ax,s16 *ay,s16 *az)
{
    u8 buf[6],res;  
...
//返回值:0,成功
//    其他,错误代码
BOOL MPU_Get_Magnetometer(s16 *mx,s16 *my,s16 *mz)
{
    u8 buf[6],res;  
