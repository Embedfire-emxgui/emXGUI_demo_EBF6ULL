...
#define configSETUP_TICK_INTERRUPT()                                                                                  \
    ...
    {                                                                                                                 \
		...
		Hook_IdleThreadStart();                                                                                       \
    } while (0)
#define configCLEAR_TICK_INTERRUPT()       \
    do                                     \
    {                                      \
        void epit_irq_clean(unsigned int epit_ch); \
        epit_irq_clean(0);                \
    } while (0)

/* Definitions that map the FreeRTOS port exception handlers to startup handler names. */
#define FreeRTOS_IRQ_Handler OS_IRQHandler//IRQ_Handler//
#define FreeRTOS_SWI_Handler SWI_Handler//SVC_Handler
