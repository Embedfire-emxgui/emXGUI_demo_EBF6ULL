...
{
	return 1;//mk_cpu_usage(0);
}

//创建线程。
static void _CreateThread(FN_ThreadEntry *Entry,void *argv,int StackSize)
{
	GUI_Thread_Create ((pfn_thread_entry)Entry,argv,StackSize,NULL,0);
}
...
static void app_RES_Maker(HWND hwnd)
{
	...
	{ //在SD卡上生成 .res文件.
		...
		if(CDlg_FileExpoler(hwnd,0,0,GUI_XSIZE,GUI_YSIZE,L"RES maker: Input DIR","B:",input))
		if(CDlg_FileExpoler(hwnd,0,0,GUI_XSIZE,GUI_YSIZE,L"RES maker: Output File","B:",out))
		{
	...
	{ //把.res文件写到Flash中.
		...
		{
			Dlg_Flash_Writer(0,0,GUI_XSIZE,GUI_YSIZE,buf,0x00000000);
		}
	}
#endif
...
static void app_loader(HWND hwnd)
{
	...
	if(thread==1) //线程已创建了?
	{
		...
		{
			app=1;
			if(CDlg_FileExpoler(hwnd,10,20,300,240,L"APP Loader","B:",buf))
			{
...
static void app_audio_player(HWND hwnd)
{
	...
	if(thread==1) //线程已创建了?
	{
		if(app==0)
		{
			...
			AudioPlayer(L"音乐播放器",0,0,600,400);
...
static struct __obj_list menu_list_1[]={
		...
		L"画图板",		&pDefIcon, 		app_palette,
		L"GUI基本测试",	&pDefIcon,   	app_GUI_DEMO,

		//L"矢量字体测试",	&pDefIcon, 			app_ttf_test,
		//L"模拟时钟",		&pDefIcon, 			app_watchtime_test,
		L"动态APP加载",	&pDefIcon, 				app_loader,
		L"Backlight",	&pDefIcon, 				app_LCD_BL_Setup,
		...
		L"Test1-7",		&pDefIcon, 				dummy,
		L"Test1-8",		&pDefIcon, 				dummy,
		L"Test1-9",		&pDefIcon, 				dummy,

		L"Test1-10",		&pDefIcon, 				dummy,
		L"Test1-11",		&pDefIcon, 				dummy,
		L"Test1-12",		&pDefIcon, 				dummy,
		L"Test1-13",		&pDefIcon, 				dummy,
		L"Test1-14",		&pDefIcon, 				dummy,
		L"Test1-15",		&pDefIcon, 				dummy,
		L"Test1-16",		&pDefIcon, 				dummy,
		L"Test1-17",		&pDefIcon, 				dummy,
		L"Test1-18",		&pDefIcon, 				dummy,
		L"Test1-19",		&pDefIcon, 				dummy,

		L"Test1-20",		&pDefIcon, 				dummy,
		L"Test1-21",		&pDefIcon, 				dummy,
		L"Test1-22",		&pDefIcon, 				dummy,
		L"Test1-23",		&pDefIcon, 				dummy,
		L"Test1-24",		&pDefIcon, 				dummy,
		L"Test1-25",		&pDefIcon, 				dummy,
		L"Test1-26",		&pDefIcon, 				dummy,
		L"Test1-27",		&pDefIcon, 				dummy,
		L"Test1-28",		&pDefIcon, 				dummy,
		L"Test1-29",		&pDefIcon, 				dummy,
...
void Board_AppMain(int x,int y,int w,int h,const WCHAR *pText)
{
	...
	hwnd	=CreateWindowEx(	WS_EX_FRAMEBUFFER|WS_EX_LOCKPOS|WS_EX_LOCKZORDER,
								&wcex,
