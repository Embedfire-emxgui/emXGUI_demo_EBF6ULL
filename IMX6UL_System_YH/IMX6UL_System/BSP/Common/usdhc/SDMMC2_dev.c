...
static BOOL sd_disk_read(U32 blk_addr,void *buf,U32 blk_count)
{
	...
	if(card_is_rdy)
	{
		...
		sd_disk_lock();
		res=SDMMC2_drvReadBlock((u8*)buf,blk_addr,blk_count);
		sd_disk_unlock();
	}
#endif
...
static BOOL	sd_disk_write(U32 blk_addr,const void *buf,U32 blk_count)
{
	...
	{
		sd_disk_lock();
		res=SDMMC2_drvWriteBlock((u8*)buf,blk_addr,blk_count);
		sd_disk_unlock();
	}
#endif
	return res;
	...
	switch(cmd)
	{
		case	BLK_DEV_GET_INFORMATION:
				...
				if(argv)
				{
					...
					pInfo->sector_count	= 100000;//g_mmc.blockCount;
...
BOOL	SDMMC2_Register(const char *name)
{
