...
static void MEM_Flush(u32 addr,u32 size)
{
...
//		PXP_EnableContinousRun(PXP,false);
		...
		SetIRQHandler(IMX_INT_PXP,pxp_isr);
		EnableIRQ(IMX_INT_PXP);
...
void PXP_Test(void)
{
	...
	DCACHE_InvalidateByRange((uint32_t)pSurf->Bits,pSurf->WidthBytes*pSurf->Height);
