...
static void _Paint(DRAWITEM_HDR *di,CTLCOLOR *cr,const WCHAR *pText)
{
	...
	switch(di->Style & LBS_ALIGNMASK) //判断文字水平对齐方式.
	{
		case LBS_LEFTTEXT:   //文字左对齐.
			dt_flag = DT_VCENTER|DT_LEFT;
			break;
		case LBS_CENTERTEXT: //文字居中对齐.
			dt_flag = DT_VCENTER|DT_CENTER;
			break;
		case LBS_RIGHTTEXT:  //文字右对齐.
			dt_flag = DT_VCENTER|DT_RIGHT;
			break;
		default:
			dt_flag = DT_VCENTER|DT_LEFT;
			break;
	...
	while(i < count) //循环绘制.
	{
		...
		{ // 绘制当前选中的Item项。
			...
			DrawText(hdc,wbuf,-1,&rc,dt_flag|DT_BORDER);
		...
		{// 绘制未选中的Item项。
			...
			SendMessage(hwnd,LB_GETTEXT,i,(LPARAM)wbuf);
			DrawText(hdc,wbuf,-1,&rc,dt_flag);
