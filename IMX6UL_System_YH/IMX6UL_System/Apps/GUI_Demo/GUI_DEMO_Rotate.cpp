...
static LRESULT	WinProc(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	//		DrawRect(hdc_mem,&rc);
			...
			SetPenColor(hdc_mem,MapARGB(hdc_mem,250,250,10,10));
			rc.w =60;//IMG_W;
			rc.h =40;//20;
			rc.x =4;
			rc.y =10;
...
#if 0
			SetPenSize(hdc_mem,1);
			SetPenColor(hdc_mem,MapARGB(hdc_mem,240,200,10,20));
			rc.w =IMG_W;
			rc.h =IMG_H;
			rc.x =0;//((IMG_W-rc.w)/2);
			rc.y =0;//((IMG_H-rc.h)/2);
			...
			SetBrushColor(hdc_mem,MapARGB(hdc_mem,250,255,10,10));
			//FillCircle(hdc_mem,180,80,45);
			FillCircle(hdc_mem,IMG_W-45,IMG_H-45,45);
...
extern "C" void	GUI_DEMO_Rotate(void)
{
	...
	hwnd	=CreateWindowEx(	WS_EX_FRAMEBUFFER,
								&wcex,
								_T("emXGUI Demo: RotateBitmap"), //窗口名称
